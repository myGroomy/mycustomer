...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const existing = await dataTable('customers').list<Record<string, unknown>>({ select: 'phone_normalized', limit: 1000 })
    ...
    if (payloads.length) await dataTable('customers').insert(payloads)
