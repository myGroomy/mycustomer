...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const order = (await dataTable('orders').list<Record<string, unknown>>({ id: `eq.${order_id}`, limit: 1 }))[0]
    ...
    await dataTable('orders').update({ id: `eq.${order_id}` }, { is_followed_up, followed_up_at })
